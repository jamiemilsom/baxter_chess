...
class PickAndPlaceMoveIt(object):
    ...
    def get_block_pose(self):
        """
        Queries the current pose of the block relative to the robot base frame.

        Returns:
            geometry_msgs.msg.Pose: The pose of the block in the base frame.
        """
        tf_buffer = tf2_ros.Buffer()
        tf_listener = tf2_ros.TransformListener(tf_buffer)

        try:
            # Replace "block" with the actual name of your block frame
            transform = tf_buffer.lookup_transform("base", "block", rospy.Time())
            return transform.transform.translation, transform.transform.rotation
        except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException) as e:
            rospy.logerr("Error getting block pose: %s" % e)
            return None, None
    ...
    def pick(self, pose):
        ...
        self._retract()
        block_pose, block_orientation = self.get_block_pose()
        if block_pose is None:
            rospy.logerr("Failed to get block pose, skipping pick.")
            return
